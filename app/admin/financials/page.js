...
export default function AdminFinancesPage() {
    ...
    const loadFinancials = async () => {
        try {
            ...
            setFinancialData(getStaticData());
        } finally {
            setLoading(false);
        }
    };
    ...
    const data = financialData || getStaticData();
        ...
        <div>
            <div className="flex items-center justify-between mb-6">
                <div>
                    ...
                    <p className="text-gray-600">Admin dashboard for financial oversight and management</p>
                </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-4 gap-4 mb-8">
                <Link
                    href="/admin/financials/add-transaction"
                ...
                >
                    ...
                    <p className="text-sm text-gray-500">Record new donation</p>
                </Link>

                <Link
                    href="/admin/financials/manage-allocations"
                ...
                >
                    ...
                    <h3 className="font-semibold text-gray-900">Manage Allocations</h3>
                    <p className="text-sm text-gray-500">Update fund distribution</p>
                </Link>

                <Link
                    href="/admin/financials/audit-log"
                ...
                <Link
                    ...
                    className="p-4 bg-white rounded-lg shadow hover:shadow-md transition border-l-4 border-gray-500"
                >
                    <Settings className="h-6 w-6 text-gray-600 mb-2" />
                    <h3 className="font-semibold text-gray-900">Settings</h3>
                    <p className="text-sm text-gray-500">Financial preferences</p>
                </Link>
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
                <div className="bg-white rounded-xl shadow p-6">
                    ...
                    <div className="text-3xl font-bold text-gray-900 mb-2">
                        {formatCurrency(data.summary.totalFunds)}
                    </div>
                    <div className="text-gray-600">Total Funds Received</div>
                    <div className="mt-4 text-sm text-gray-500">
                        <span className="font-medium">{data.summary.totalDonations.toLocaleString()}</span> donations
                    </div>
                    <div className="mt-2">
                ...
                <div className="bg-white rounded-xl shadow p-6">
                    ...
                    <div className="text-3xl font-bold text-gray-900 mb-2">
                        {formatCurrency(data.summary.fundsThisMonth)}
                    </div>
                    <div className="text-gray-600">Funds This Month</div>
                    <div className="mt-4 text-sm text-gray-500">
                        Avg: {formatCurrency(data.summary.avgDonation)} per donation
                    </div>
                    <div className="mt-2">
                ...
                <div className="bg-white rounded-xl shadow p-6">
                    ...
                    <div className="text-3xl font-bold text-gray-900 mb-2">
                        {formatCurrency(data.donationSources?.[0]?.amount || 0)}
                    </div>
                    ...
                    <div className="mt-4">
                        <div className="h-2 bg-gray-200 rounded-full overflow-hidden">
                            <div
                                ...
                                style={{ width: `${data.donationSources?.[0]?.percentage || 0}%` }}
                            ></div>
                        </div>
                        <div className="text-sm text-gray-500 mt-1">{data.donationSources?.[0]?.percentage || 0}% of total</div>
                    </div>
                </div>
                ...
                <div className="bg-white rounded-xl shadow p-6">
                    ...
                    <div className="text-3xl font-bold text-gray-900 mb-2">
                        {formatCurrency(data.donationSources?.[1]?.amount || 0)}
                    </div>
                    ...
                    <div className="mt-4">
                        <div className="h-2 bg-gray-200 rounded-full overflow-hidden">
                            <div
                                ...
                                style={{ width: `${data.donationSources?.[1]?.percentage || 0}%` }}
                            ></div>
                        </div>
                        <div className="text-sm text-gray-500 mt-1">{data.donationSources?.[1]?.percentage || 0}% of total</div>
                    </div>
                </div>
            </div>
                ...
                <div>
                    <div className="bg-white rounded-xl shadow p-6 mb-8">
                        ...
                        <div className="space-y-4">
                            {Object.entries(data.allocation).map(([category, amount]) => {
                                const percentage = (amount / data.summary.totalFunds * 100).toFixed(1);
                        ...
                        <div className="mt-6 pt-6 border-t border-gray-200">
                            <div className="flex justify-between">
                                <div>
                                    ...
                                    <div className="text-lg font-bold text-gray-900">{formatCurrency(data.summary.allocatedAmount)}</div>
                                </div>
                                <div>
                                    <div className="text-sm text-gray-600">Pending</div>
                                    <div className="text-lg font-bold text-gray-900">{formatCurrency(data.summary.pendingAmount)}</div>
                                </div>
                            </div>
                        </div>
                    </div>
                    ...
                    <div className="bg-white rounded-xl shadow p-6">
                        ...
                        <div className="space-y-4">
                            <div className="flex items-center justify-between">
                                ...
                                <div className="font-medium">{formatCurrency(data.summary.avgDonation)}</div>
                            </div>
                            <div className="flex items-center justify-between">
                                <div className="text-gray-600">Total Donations</div>
                                <div className="font-medium">{data.summary.totalDonations.toLocaleString()}</div>
                            </div>
                            <div className="flex items-center justify-between">
                                <div className="text-gray-600">This Year</div>
                                <div className="font-medium">{formatCurrency(data.summary.fundsThisYear)}</div>
                            </div>
                            <div className="flex items-center justify-between">
                                <div className="text-gray-600">Growth Rate</div>
                                <div className="font-medium text-green-600">{data.summary.growthRate}%</div>
                            </div>
                        </div>
                    </div>
                </div>
            ...
            {/* Recent Transactions - Admin View */}
